...
$("#__nuSheet button").click(function() {
	...
	{
		const code = `
$("#__nuPrompt button").click(function() {
	nu.prompt("お名前を入力してください", 
		{ placeholder: "山田太郎", value: "", type: "text" }, 
		{
			onOk: (val) => {
				if (!val) { nu.toast.w("未入力です"); return; }
				nu.toast.s("こんにちは、"+ val +"さん！");
			},
			onCancel: () => nu.toast.i("キャンセルしました")
		}
	);
});
            `.trim();

		$("#__nuPrompt pre").text(code); // 表示用
		new Function(code)(); // 実行用
	}
